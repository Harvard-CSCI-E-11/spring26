...
class E11Ssh:
    ...
    def __init__(self, hostname, username="ubuntu", port=22, key_filename=None, pkey_pem=None, timeout=10):
        ...
        try:
            ...
            self._ssh.connect(hostname=hostname, port=port, username=username, key_filename=key_filename,
                         pkey=pkey, timeout=timeout, banner_timeout=timeout, auth_timeout=timeout)
            LOGGER.debug("connect successful")
        except (OSError,TimeoutError) as e:
            LOGGER.error("OSError e=%s hostname=%s port=%s username=%s",e,hostname,port,username)
            raise
        self._sftp =None
        self._cwd = None
