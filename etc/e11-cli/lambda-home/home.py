...
COOKIE_SECURE = True
COOKIE_SAMESITE = os.environ.get("COOKIE_SAMESITE", "Strict")  # Lax|Strict|None
SESSION_TTL_SECS = int(os.environ.get("SESSION_TTL_SECS", str(60*60*24*180)))  # 180 days
COOKIE_DOMAIN='csci-e-11.org'
...
def _textresp(status: int, body: str, headers: Dict[str, str] = None, cookies: Dict[str, str]=None) -> Dict[str, Any]:
    return {
        "statusCode": status,
...
def do_home_page(event, context, status="",extra=""):  # pylint: disable=unused_argument
    ...
    return _textresp(200, template.render(harvard_key=url, status=status, extra=extra))
...
def do_dashboard(event, context): # pylint: disable=unused_argument
    ...
    return _textresp(200, template.render())
...
def do_logout(event, context):# pylint: disable=unused_argument
    ...
    (url, issued_at) = oidc.build_oidc_authorization_url_stateless(openid_config = get_odic_config())
    LOGGER.info("url=%s issued_at=%s ",url,issued_at)
    return _textresp(200, env.get_template("logout.html").render(harvard_key=url), cookies=[del_cookie])
